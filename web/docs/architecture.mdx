...
The original view of the Unicode Standard keeps the variants of a character in the character layer and treats them as free variants of writing: a character is first turned into another character by a variation selector, and that character is then turned, by cursive joining and the rest of shaping, into the joining variants that are its glyphs in the glyph layer. [UTR \#54, “Unicode Mongolian 12.1 Snapshot”](https://www.unicode.org/reports/tr54/) maintained the variants in this way, and its data file is [`StandardizedVariants.txt`](https://www.unicode.org/Public/draft/ucd/StandardizedVariants.txt). For example, the file records

```
1820 180B; second form; isolate initial medial final # MONGOLIAN LETTER A
```

in which `1820 180B` is a variation sequence: `1820` is the base character that stands for MONGOLIAN LETTER A, and `180B` the variation selector, and the description “second form” still belongs to the character layer. The sequence then has the four joining variants isolate, initial, medial, and final, which belong to the glyph layer. As the Hudum chapter shows, however, the four joining variants of `1820 180B` do not share a common abstract form—their written units are <MNG links="A" />, <MNG links="A" />, <MNG links="A A" />, and <MNG links="Aa" />. The joining variants of a Mongolian character are not maintained with such an intent, and this view does not hold.
...
- **The variant set of a letter depends on the writing system.** The Mongolian block unifies letters across Hudum, Todo, Sibe, and Manchu and their Ali Gali extensions. These writing systems do not agree on the variant used at every joining position, and a writing system that has no form at a position must render a virtual form derived from another position. A writing-system-neutral list cannot express these differences.
