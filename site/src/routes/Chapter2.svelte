...
<p class="en">Each writing system using Mongolian script and the unified writing system formed by integrating them have the same architecture of text representation and text shaping. The text representation and text shaping rules contained in the individual writing systems and the integrated writing system are the concrete realization of this common architecture. This chapter describes this common architecture, and subsequent chapters describe the specific rules of each writing system in detail.</p>
<p class="zh">使用蒙古文字的各个书写系统，以及综合这些书写系统所形成的统一的书写系统，均具有相同的文本表记与文本成形架构。每种书写系统及其综合系统所包含的文本表记与文本成形规则，都是该共同架构的具体实现。本章将介绍这一共同架构，后续章节将详细说明各个书写系统的具体规则。</p>

<h3>
  2.1 <a name="Data files and the standard toolchain">Data files and the standard toolchain</a>
</h3>

<p class="en">The text shaping data for Mongolian script are in the form of machine-readable files that provide the character attribute information, text shaping rules, font generation program, and font testing program for Mongolian script.</p>
<p class="zh">蒙古文字的文本成形数据以机器可读的文件形式，提供蒙古文字的字符属性信息、文本成形规则信息、字库生成程序和字库测试程序等内容。</p>
...
<p class="zh">蒙古文字的字符属性信息和文本成形规则信息，详细描述了各书写系统中蒙古文字字符与字形的对应关系，以及蒙古文字字符串与字形序列的对应关系。字库生成程序在提供基础字库文件、字形图像与字形的对应关系信息和设定信息的情况下，能够增补并生成可用的字库。字库测试程序则在提供字库文件和字形图像与字形对应关系信息的情况下，能够测试字库对各书写系统的文本成形规则的支持情况。</p>

<h3>
  2.2 <a name="Character set">Character set</a>
</h3>

<p class="en">The characters included in each writing system of the Mongolian script can be categorized into Mongolian-specific characters and characters shared with other scripts.</p>
...
<p class="en">
  The specification used in this manual is based on the phonemic model, in which the characters correspond to the phonemes of the written language as recorded in the writing systems, with some compromises to maintain relative stability with the existing system. For example, the final <small>Ï</small> in Manchu records the phoneme /ɹ̩/, is the same as the phonetic letter <i>ï</i>, but because it corresponds to ⟨i⟩ rather than ⟨y⟩ in the Möllendorff transcription, it is treated as corresponding to the phonetic letter <i>i</i>
...
<p class="en">
  ...
  <small>N</small> when it appears as an onset and as <small>A</small> when it appears as a coda (except for loanwords such as <small>SWK2OIND</small>); and the grapheme sequence <small>AORDU</small> may correspond to <i>ordo</i>
